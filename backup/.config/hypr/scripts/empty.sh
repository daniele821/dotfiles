...
EMPTY="${FIRST_EMPTY}"
SPECIAL_WS="$(hyprctl clients -j | jq '.[] | select(.focusHistoryID == 0) | .workspace.id < 0')"
for n in ${EXTRA_WS}; do [[ $n -eq $EMPTY ]] && ((EMPTY = EMPTY + 1)); done

case "$1" in
"goto")
    [[ "$CURRENT_ID" -lt "$EMPTY" && "$CURRENT_ID" -ge "$FIRST_EMPTY" && "$CURRENT_WINDOWS" -eq 0 ]] && EMPTY="$CURRENT_ID"
    hyprctl dispatch workspace "${EMPTY}"
    ;;
"moveto")
    eval "${SPECIAL_WS}" && [[ "$CURRENT_ID" -lt "$EMPTY" && "$CURRENT_ID" -ge "$FIRST_EMPTY" && "$CURRENT_WINDOWS" -eq 0 ]] && EMPTY="$CURRENT_ID"
    ! eval "${SPECIAL_WS}" && [[ "$CURRENT_ID" -lt "$EMPTY" && "$CURRENT_ID" -ge "$FIRST_EMPTY" && "$CURRENT_WINDOWS" -le 1 ]] && EMPTY="$CURRENT_ID"
    hyprctl dispatch movetoworkspace "${EMPTY}"
    ;;
"sendto")
    eval "${SPECIAL_WS}" && [[ "$CURRENT_ID" -lt "$EMPTY" && "$CURRENT_ID" -ge "$FIRST_EMPTY" && "$CURRENT_WINDOWS" -eq 0 ]] && EMPTY="$CURRENT_ID"
    ! eval "${SPECIAL_WS}" && [[ "$CURRENT_ID" -lt "$EMPTY" && "$CURRENT_ID" -ge "$FIRST_EMPTY" && "$CURRENT_WINDOWS" -le 1 ]] && EMPTY="$CURRENT_ID"
    hyprctl dispatch movetoworkspacesilent "${EMPTY}"
    ;;
"left")
